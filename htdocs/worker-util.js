// worker-util.js
// Socket communication helpers for the container worker.
// Source: container2wasm v0.8.4 examples/wasi-browser/htdocs/worker-util.js (verbatim)
...
function wasiHackSocket(wasi, listenfd, connfd) {
    ...
    wasi.wasiImport.fd_prestat_get = (fd, prestat_ptr) => {
        if ((fd == listenfd) || (fd == connfd)) {
            // Reserve these fds from the preopen scan without claiming as dirs.
            let buffer = new DataView(wasi.inst.exports.memory.buffer);
    ...
    wasi.wasiImport.sock_send = (fd, iovs_ptr, iovs_len, si_flags, nwritten_ptr) => {
        if (fd != connfd) {
        ...
        for (var i = 0; i < iovecs.length; i++) {
            ...
            if (ret == errStatus) return ERRNO_INVAL;
            wtotal += buf.length;
        }
    ...
    wasi.wasiImport.sock_recv = (fd, iovs_ptr, iovs_len, ri_flags, nread_ptr, ro_flags_ptr) => {
        ...
        if (sockreadable == false) return ERRNO_AGAIN;
        var buffer = new DataView(wasi.inst.exports.memory.buffer);
        var buffer8 = new Uint8Array(wasi.inst.exports.memory.buffer);
        ...
        for (var i = 0; i < iovecs.length; i++) {
            ...
            if (data == errStatus) return ERRNO_INVAL;
