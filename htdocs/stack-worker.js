// stack-worker.js — Web Worker that runs c2w-net-proxy.wasm (the browser network proxy).
// Source: container2wasm v0.8.4 examples/wasi-browser/htdocs/stack-worker.js (verbatim)
...
function wasiHack(wasi, certfd, connfd) {
    ...
    wasi.wasiImport.poll_oneoff = (in_ptr, out_ptr, nsubscriptions, nevents_ptr) => {
        ...
        let isReadPollConn = false, isClockPoll = false;
...
function envHack(wasi) {
    return {
        http_send: function(addressP, addresslen, reqP, reqlen, idP) {
            ...
            streamCtrl[0] = 0;
            postMessage({ type: "http_send", address: address.slice(), req: req.slice() });
        ...
        http_writebody: function(id, bodyP, bodylen, nwrittenP, isEOF) {
            ...
            streamCtrl[0] = 0;
            postMessage({ type: "http_writebody", id: id, body: body.slice(), isEOF: isEOF });
            Atomics.wait(streamCtrl, 0, 0);
            if (streamStatus[0] < 0) return ERRNO_INVAL;
